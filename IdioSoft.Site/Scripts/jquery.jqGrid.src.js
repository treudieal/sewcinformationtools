						...
						if(p.autosearch===true){
							if(p.searchOnEnter) {
								$("input",thd).keypress(function(e){
									var key = e.charCode || e.keyCode || 0;
									if (key === 13) {
									    $.cookies.set('ckgs_' + cm.name, $(e.target).val());
										triggerToolbar();
										return false;
									}
									return this;
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				if(options.dataUrl !== undefined) {
					...
					var rowid = options.name ? String(options.id).substring(0, String(options.id).length - String(options.name).length - 1) : String(options.id);
                    try{var postData = options.postData || ajaxso.postData;}catch (m){}
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			// dropdown for: choosing group operator type
			    var groupOpSelect = $("<select class='opsel ui-Search-select jqSearchselect'></select>");
			...
			if(this.p.groupButton) {
			    inputAddSubgroup = $("<input type='button' value='+ {}' title='Add subgroup' class='add-group ui-state-default ui-corner-all' style='width:60px;vertical-align:top;min-width:60px;'/>");
			...
			// button for adding a new rule
			    var inputAddRule = $("<input type='button' value='+' title='Add rule' class='add-rule ui-add ui-state-default ui-corner-all' style='width:30px;vertical-align:top;min-width:30px;'/>"), cm;
			...
			if (parentgroup !== null) { // ignore the first group
			    var inputDeleteGroup = $("<input type='button' value='-' title='Delete group' class='delete-group ui-state-default ui-corner-all' style='min-width:30px;width:30px; padding-top:-10px;margin-top:-10px;'/>");
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			var ruleFieldSelect = $("<select class='ui-Search-select jqSearchselect'></select>"), ina, aoprs = [];
			...
			var ruleOperatorSelect = $("<select class='selectopts ui-Search-option jqSearchselect'></select>");
			...
			if(this.p.ruleButtons === true) {
			    var ruleDeleteInput = $("<input type='button' value='-' title='Delete rule' class='delete-rule ui-del ui-state-default ui-corner-all' style='min-width:30px;width:30px; padding-top:-10px;margin-top:0px;'/>");
