...
namespace IdioSoft.Site.ClassLibrary
{
    public class Sms
    {
        IdioSoft.Business.Method.SQLDbHelper objDbSMSSQLAccess = new SQLDbHelper(ConfigurationManager.ConnectionStrings["SMSConnString"].ToString());
        //插入一条短信
        public Boolean subDoSendMessage(int ExpressLevel, string Sender, string ReceiverMobileNo, string Msg, string SendTime, int IsChinese, int CommPort, int NeedReport)
        {
            ...
            for (int i = 0; i <= ary.Count - 1; i++)
            {
                ...
                strError = objDbSMSSQLAccess.funString_SQLExecuteNonQuery(strSQL);
            }
            if (strError == "")
            {
                return true;
